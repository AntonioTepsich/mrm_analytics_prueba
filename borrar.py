...
def extract_text_with_ocr(pdf_path):
    ...
    for page_num in range(len(doc)):
        ...
        # Convertir la imagen PIL a un array de OpenCV
        open_cv_image = np.array(cropped_img)

        # Aplicar un filtro Gaussian Blur para suavizar la imagen
        blurred = cv2.bilateralFilter(open_cv_image, d=6, sigmaColor=15, sigmaSpace=15)
        ...
        thresh1.show()
        # Aplicar OCR al número suavizado y binarizado
        custom_config = r'--oem 3 --psm 3'  
        text = pytesseract.image_to_string(thresh, config=custom_config, lang="eng")

        # Verificar si el texto coincide con el formato esperado (e.g., letras y números)
        # if text.strip().isalnum():
        pattern = r'\d{2}[A-Z]{2}\s[A-Z]{2}\s\d{4}'
        matches = re.findall(pattern, text)
        print(f"Case No.: {matches[0] if matches else 'No Case found'}")
        print(text)
        break
